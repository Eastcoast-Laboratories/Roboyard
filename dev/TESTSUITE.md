...
| [e] `HistoryCompletionE2ETest` | ⏳ Pending                | 5     | Random game completion → history entry shows "Completions: 1". Also verifies that after moving all 4 robots and saving to slot 1, the save-slot info popup resolves the matching history entry for the same map. | e2e, history, random-game, completion-tracking, save-slot, map-signature, espresso              |
| [e] `HistorySyncE2ETest`      | ✅ Passing                | 1     | Full sync round-trip: register → play random game → auto-upload → verify server → reset all data → re-login → download → verify restored fields (optimalMoves, maxHintUsed, everUsedHints, movesMade, bestTime, bestMoves, completionCount, completionTimestamps). Includes timezone validation. | e2e, history, sync, register, login, upload, download, round-trip, timezone     |
