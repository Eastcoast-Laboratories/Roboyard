...
| `Level1FastE2ETest` [e]        | ❌ Failing                | 1     | Complete Level 1 in under 10s — tests speedrun achievements.                                                       | e2e, level-game, achievement, solver, fast, speedrun, timer           |
| `Level1SlowE2ETest` [e]        | ❌ Failing                | 1     | Complete Level 1 in over 30s — verifies only `level_1_complete` is unlocked (no speedrun).                         | e2e, level-game, achievement, slow, timer                             |
| `Level1WrongE2ETest` [e]       | ❌ Failing                | 1     | Execute wrong moves — verifies level is NOT completed.                                                             | e2e, level-game, negative-test, wrong-moves                           |
| `Level3E2ETest` [e]            | ❌ Failing                | 1     | Complete levels 1–3 using solver. Tests level completion and achievements.                                         | e2e, level-game, achievement, solver                                  |
| `Level10E2ETest` [e]           | ❌ Failing                | 1     | Complete levels 1–10 to unlock `3_star_10_levels` achievement.                                                     | e2e, level-game, achievement, 3-star, solver                          |
...
| `GameModeMemoryE2ETest` [e]    | ❌ Failing                | 1     | After completing Level 1 and navigating to Achievements, "Next Level" button still visible. No auto-regeneration.  | e2e, game-mode, level-game, achievements, navigation, next-level      |
| `HistoryCompletionE2ETest` [e]  | ❌ Failing (2/5)          | 5     | Random game completion → history entry shows "Completions: 1". Also verifies that after moving all 4 robots and saving to slot 1, the save-slot info popup resolves the matching history entry for the same map. | e2e, history, random-game, completion-tracking, save-slot, map-signature, espresso              |
| `HistorySyncE2ETest` [e]       | ⚠️ Requires network      | 1     | Full sync round-trip: register → play random game → auto-upload → verify server → reset all data → re-login → download → verify restored fields (optimalMoves, maxHintUsed, everUsedHints, movesMade, bestTime, bestMoves, completionCount, completionTimestamps). Includes timezone validation. | e2e, history, sync, register, login, upload, download, round-trip, timezone     |
| `HistoryReplayUpdateE2ETest` [e]  | ✅ Passing                | 1     | Play random game with extra moves, replay from history with optimal moves → bestMoves updated, completionCount=2. Proves history entries are correctly updated on replay. | e2e, history, random-game, replay, bestMoves, completion-tracking     |
...
| `LevelScreenMinimapE2ETest` [e]  | ✅ Passing                | 3     | After completing Level 1, level selection shows minimap + info-button; info popup shows "Completions: 1"; boardSize not hardcoded 16x16. | e2e, level-screen, minimap, info-button, history, level-game, solver, espresso |
...
| `RandomGameHistoryMinimapTest` [e]  | ✅ Passing                | 1     | Start random game, make move, verify history minimap is displayed correctly (not dummy blue placeholder). Proves minimaps work for real games. | e2e, history, random-game, minimap, espresso                          |
| `LiveMoveCounterE2ETest` [e]   | ❌ Failing                | 4     | Live Move Counter: hint system regression, eye toggle, live counter after move.                                    | e2e, hint-system, live-move-counter, level-game, regression           |
...
| `ReverseMoveUndoBugTest` [e]   | ❌ Failing                | 1     | Regression: after reverse-move undo, subsequent robot moves still work.                                            | regression, undo, reverse-move, level-game, bug-fix                   |
...
| `ImpossibleDifficultyNewGameTest` [long] [e]   | ⏳ Not tested | 1     | Multi-target mode (2 robots, 2 targets) + Impossible difficulty. Plays 25 consecutive games with 20s wait. Visible E2E test. Verifies no OOM crash. | e2e, multi-target, impossible, solver, oom, new-game, memory, stress-test |
| `LevelSelectionLandscapeTest` [e]  | ❌ Failing            | 4     | Landscape orientation tests for level selection. Verifies: (1) level selection loads in landscape without crash, (2) scroll_up_arrow null check prevents crash, (3) card binding works correctly, (4) starting a level in landscape mode. Tests orientation handling and layout robustness. | e2e, landscape, level-selection, orientation, ui, null-safety |
