...
| `SaveLoadCombinationsE2ETest` | 🆕 New                   | 8     | Save in all edge-case combos: fresh, after hints, live-move-counter, after moves, hints+moves, load→re-save, history-load→save, level-game→save. Verifies no ❌ red cross. | save, load, target-validation, hints, move-counter, history, level-game, edge-cases |
