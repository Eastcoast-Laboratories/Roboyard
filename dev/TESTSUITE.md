...
| `YubaxCrashReproductionTest` [e]     | ⏳ Pending               | 1     | Reproduces crash from GitHub Issue #67 on YUBAX map (board:12,14). Loads exact map via deep-link, executes crash-inducing move sequence: pink up/left/up/right, blue up/left/up/right/up/left. Verifies no crash. | e2e, crash-reproduction, deep-link, yubax, issue-67, espresso                |
...
| `GameScreenSessionTest` | ✅ Passing | 2 | GameSession-based game screen: random game and level game (via `levelItem_1` testTag) must render the board UI ("Moves:" counter) instead of staying on "Calculating...". Run via `:composeApp:desktopTest --tests "roboyard.ui.compose.GameScreenSessionTest"`. | compose, desktop, ui-test, game-session, regression |
...
| `HistoryAutosaveTest` | ✅ Passing | 9 | History autosave and hint tracking. Fixed two tests that contradicted the intended chronological hint semantics (clean-solve timestamps persist; hints before first solve disqualify). | compose, desktop, history, autosave |
...
| `MapGenerationValidatorTest` | ✅ Passing | 6 | Shared `MapGenerationValidator` retry/fallback decisions: accept valid, TOO_EASY/TOO_HARD/NO_SOLUTION/TRIVIAL rejection reasons, retry until maxAttempts then accepted fallback (incl. repeated solver errors), attempt counting and reset, positive maxAttempts requirement. Run via `:shared:desktopTest --tests "roboyard.logic.core.MapGenerationValidatorTest"`. | shared, map-generation, unit-test |
| `MoveSafetyTest` | ✅ Passing | 4 | Shared `Move` path-construction bounds: identical/two-changed/non-aligned states throw IllegalArgumentException; valid horizontal/vertical moves build finite path maps. Run via `:shared:desktopTest --tests "driftingdroids.model.MoveSafetyTest"`. | shared, solver, unit-test |
| `SolverDDTest` | ✅ Passing | 4 | Shared solver bridge: `SolverDD` solves a 1-move map (status `solved`, solution list, `isSolution01`), cancel-before-run yields `noSolution`, `RRGetMap.createDDWorld` throws on missing target, ASCII map generate/parse roundtrip preserves robot+target positions. Run via `:shared:desktopTest --tests "roboyard.logic.solver.SolverDDTest"`. | shared, solver, unit-test |
| `BoardConversionValidationTest` | ✅ Passing | 5 | `gridElementsToBoard` rejects incomplete, duplicate, unsupported-silver and invalid robot placements as well as missing or unusable goals instead of producing boards that crash the solver. Run via `:composeApp:desktopTest --tests "roboyard.ui.compose.BoardConversionValidationTest"`. | compose, desktop, board-conversion, unit-test |
| `PathHistoryResetTest` | ✅ Passing | 2 | `startLevelGame` and `resetMoveCountsAndHistory` must clear `session.pathHistory` so robot trails from the previous game are not redrawn on the new board (Android parity: `GameGridView.clearRobotPaths`). Run via `:composeApp:desktopTest --tests "roboyard.ui.compose.PathHistoryResetTest"`. | compose, desktop, path-history, trails, regression, unit-test |
| `SoundManagerSingletonTest` | ✅ Passing | 1 | `getSoundManager()` must return the same shared instance so pause/resume/volume calls reach the playing clip (Android parity: `SoundManager.getInstance` singleton). Run via `:composeApp:desktopTest --tests "roboyard.ui.compose.SoundManagerSingletonTest"`. | compose, desktop, sound, volume, regression, unit-test |
| `GameSessionTest` | ✅ Passing | 11 | Shared `GameSession` core lifecycle with in-memory `PlatformStorage`: `setGameState` exposes state via `StateFlow`, slide movement to boundary and move counting, move rejection without selected robot, undo restores robot position and decrements moves, empty-history undo fails, `setGameComplete` marks state, `saveGame` writes `saves/save_N.dat`, slot-0 manual-save protection vs. allowed autosave, `handleGridTouch` robot selection, effective difficulty delegation, `extractMetadataFromSaveData` delegation to `GameState`. Run via `:shared:desktopTest --tests "roboyard.logic.managers.GameSessionTest"`. | shared, game-session, unit-test |
...
| `StreakManagerTest`               | ✅ Passing           | 10    | StreakManager daily login tracking, streak increments/resets, sync date getter regression, achievement integration.                                 | streak, daily-login, achievement-manager, sync, instrumented |
