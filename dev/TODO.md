...
er zeigt noch keine miinimalps und keinen info button mit popup im level screen an, analysiere das , fixe es, dann baue einen unittest, der den levelscreen aufruft anhand eines existierenden aus @TESTSUITE.md#L68-73 , etc und verifiziere, dass die minimaps angezeigt werden und der info button mit popup.

es könnte daran liegen, dass die levels noch nicht richtig in der history gespeichert werden, die landen zwar schon mit der richtigen minimap in der history, aber es fehlt bei den levels die completion data und wie oft man den gespielt hat und die boardzize, da steht immer completed: 0 und boardsize 16x16, was falsch ist, (btw: ( entferne den defualt  von 16x16 wenn keine boardsize vorhanden keine fallbacks!!!)
