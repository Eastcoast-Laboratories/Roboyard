...
class SaveLoadTest {
    ...
    @Test
    fun testSaveAndLoadSlot2() {
        println("[TEST] Starting save/load test for slot 2")
        
        // Create a simple board
        val board = Board.createBoardFreestyle(null, 6, 6, 2) ?: run {
            println("[TEST] Failed to create board")
            return
        }
        board.setRobots(intArrayOf(15, 20)) // Robot 0 at (3, 2), Robot 1 at (2, 3)
        board.addGoal(9, 0, 0) // Goal at (3, 1) for robot 0
        board.addGoal(14, 1, 1) // Goal at (2, 2) for robot 1
        board.setGoalRandom()
        
        println("[TEST] Created board: ${board.width}x${board.height}")
        println("[TEST] Robot positions: ${board.robotPositions.joinToString(",")}")
        println("[TEST] Goal positions: ${board.goals.map { it.position }.joinToString(",")}")
        ...
        // Save game to slot 2
        val saveFileName = "saves/save_2.dat"
        println("[TEST] Saving game to: $saveFileName")
        ...
        println("[TEST] Loaded board: ${loadedBoard.width}x${loadedBoard.height}")
        println("[TEST] Loaded robot positions: ${loadedBoard.robotPositions.joinToString(",")}")
        
        // Verify loaded board matches original
        assertEquals(board.width, loadedBoard.width, "Board width should match")
        assertEquals(board.height, loadedBoard.height, "Board height should match")
        assertEquals(board.robotPositions[0], loadedBoard.robotPositions[0], "Robot 0 position should match")
        assertEquals(board.robotPositions[1], loadedBoard.robotPositions[1], "Robot 1 position should match")
        
        println("[TEST] Save/load test for slot 2 completed successfully - all data matches")
