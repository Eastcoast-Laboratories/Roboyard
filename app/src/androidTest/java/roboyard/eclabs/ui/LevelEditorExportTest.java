...
public class LevelEditorExportTest {
    ...
    public void testExportDialogShowsSaveToSourcecodeButton() throws InterruptedException {
        Timber.d("[UNITTESTS][TEST_EXPORT] Starting Save to Sourcecode button visibility test");

        // Step 1: Navigate to Settings
        Timber.d("[UNITTESTS][TEST_EXPORT] Step 1: Clicking Settings button");
        onView(withId(R.id.settings_icon_button)).perform(click());
        Thread.sleep(1000);

        // Step 2: Long press title for 3.5s to open Debug Settings
        Timber.d("[UNITTESTS][TEST_EXPORT] Step 2: Long pressing title for 3.5 seconds");
        onView(withId(R.id.title_text)).perform(longPressFor(3500));
        Thread.sleep(1000);

        // Step 3: Open Level Editor
        Timber.d("[UNITTESTS][TEST_EXPORT] Step 3: Opening Level Editor");
        onView(withText("Open Level Editor")).perform(click());
        Thread.sleep(2000);

        // Step 4: Verify editor is displayed
        Timber.d("[UNITTESTS][TEST_EXPORT] Step 4: Verifying Level Editor is displayed");
        ...
        Timber.d("[UNITTESTS][TEST_EXPORT] Step 5: Clicking Export Level Text button");
        ...
        Timber.d("[UNITTESTS][TEST_EXPORT] Step 6: Verifying Export dialog is displayed");
        onView(withText("Copy to Clipboard")).check(matches(isDisplayed()));
        Timber.d("[UNITTESTS][TEST_EXPORT] Export dialog displayed with Copy to Clipboard button");

        // Step 7: Wait for receiver check (3s timeout + margin)
        Timber.d("[UNITTESTS][TEST_EXPORT] Step 7: Waiting for receiver check (4 seconds)");
        Thread.sleep(4000);
        ...
        try {
            ...
            Timber.d("[UNITTESTS][TEST_EXPORT] Save to Sourcecode button IS visible - receiver is reachable");

            // Step 9: Wait 4 seconds so the user can see the button on screen
            Timber.d("[UNITTESTS][TEST_EXPORT] Step 9: Waiting 4 seconds so button is visible on screen");
            Thread.sleep(4000);

            // Step 10: Click Save to Sourcecode
            Timber.d("[UNITTESTS][TEST_EXPORT] Step 10: Clicking Save to Sourcecode");
            onView(withText("\uD83D\uDCBE Save to Sourcecode")).perform(click());
            Thread.sleep(3000);

            // Step 10: Verify button text changed to success
            Timber.d("[UNITTESTS][TEST_EXPORT] Step 10: Verifying save succeeded");
            onView(withText("\u2713 Saved to Sourcecode")).check(matches(isDisplayed()));
            Timber.d("[UNITTESTS][TEST_EXPORT] SAVE TO SOURCECODE TEST PASSED - level saved successfully!");
    ...
    public void testExportDialogAlwaysHasCopyButton() throws InterruptedException {
        Timber.d("[UNITTESTS][TEST_EXPORT] Starting Export dialog basic test");
        ...
        Timber.d("[UNITTESTS][TEST_EXPORT] EXPORT DIALOG BASIC TEST PASSED");
    }
