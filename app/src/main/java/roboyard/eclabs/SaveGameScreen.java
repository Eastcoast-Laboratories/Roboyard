...
public class SaveGameScreen extends GameScreen {
    ...
    public void refreshSaveSlot(int slotNumber) {
        ...
        // Find and refresh the specific button
        for (IGameObject element : instances) {
            if (element instanceof GameButtonLoadGame) {
                GameButtonLoadGame loadButton = (GameButtonLoadGame) element;
                if (loadButton.getButtonNumber() == slotNumber) {
                    // Clear cache for this map and refresh the button
                    clearCachesForMap(mapPath);
                    loadButton.create(); // This will reload from file with cleared cache
                    Timber.d("[MINIMAP] Refreshed load button for slot: %d", slotNumber);
                    break;
                }
            } else if (element instanceof GameButtonGotoSavedGame) {
                GameButtonGotoSavedGame saveButton = (GameButtonGotoSavedGame) element;
                if (saveButton.getButtonNumber() == slotNumber) {
                    // For save buttons, just clear cache - no need to refresh them
                    clearCachesForMap(mapPath);
                    Timber.d("[MINIMAP] Cleared cache for save button slot: %d", slotNumber);
                    break;
                }
            }
        }
